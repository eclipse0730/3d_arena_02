...
### 2026-03-25 - README 문서 운영 규칙 추가

작업 내용:
- 프로젝트 루트에 `README.md`를 새로 만들고, 문서 운영 규칙을 한눈에 볼 수 있도록 정리했다.
- `DevelopmentPlan.md`는 개발 시작 전에 개발 단계 플랜을 먼저 생성하는 문서라는 점을 명시했다.
- `DevelopmentProgress.md`는 개발 중 소스가 추가되거나 변경될 때마다 기록하는 문서라는 점을 명시했다.
- 실제 작업 흐름이 바로 보이도록 계획 작성, 진행 기록, 마무리 확인 순서를 README에 함께 정리했다.

수정한 주요 파일:
- `README.md`

확인 내용:
- 루트 README에서 `DevelopmentPlan.md`와 `DevelopmentProgress.md`의 역할과 사용 시점을 바로 확인할 수 있게 정리했다.

다음 작업:
- 이후 문서 운영 규칙이 바뀌면 README와 진행 기록 문서를 함께 갱신한다.

### 2026-03-25 - README 상세 정리

작업 내용:
- 루트 `README.md`를 한 번 더 다듬어 프로젝트 소개, 현재 구현 범위, 실행 방법, 주요 폴더 구조, 자주 보는 스크립트, 작업 규칙, 검증 방법까지 포함하도록 확장했다.
- 새 팀원이 들어와도 어떤 씬을 열어야 하는지, 어떤 문서를 먼저 봐야 하는지, 어떤 스크립트를 자주 건드리게 되는지 바로 파악할 수 있게 구성했다.
- 기존에 넣어둔 문서 운영 규칙은 유지하면서, 실제 개발과 검증 흐름에 바로 도움이 되는 정보 위주로 재정리했다.

수정한 주요 파일:
- `README.md`

확인 내용:
- README에서 `DevelopmentPlan.md`와 `DevelopmentProgress.md`의 역할뿐 아니라 실행 방법과 기본 검증 절차까지 한 번에 확인할 수 있게 정리했다.

다음 작업:
- 프로젝트 구조나 핵심 스크립트 위치가 바뀌면 README의 폴더 구조와 자주 보는 스크립트 섹션도 함께 갱신한다.

### 2026-03-25 - 플레이어 머리 위 이름 크기 조정

작업 내용:
- 플레이어 머리 위에 표시되는 이름 텍스트의 폰트 크기를 더 잘 보이도록 `28`로 올렸다.
- 폰트 크기 증가에 맞춰 이름 텍스트 영역 높이도 약간 늘려, 글자가 위아래로 잘리지 않도록 함께 보정했다.
...
확인 내용:
- `dotnet build Assembly-CSharp.csproj` 빌드가 경고 0개, 오류 0개로 통과했다.
- 코드상으로 머리 위 이름 UI의 `fontSize`가 `28`로, 텍스트 영역 높이가 `36`으로 반영된 것을 확인했다.

다음 작업:
- Unity Play 모드에서 실제 카메라 거리 기준으로 이름 크기가 너무 크거나 겹치지 않는지 한 번 더 확인한다.

### 2026-03-25 - 플레이어 머리 위 이름 크기 재조정

작업 내용:
- 머리 위 이름 텍스트가 조금 더 잘 보이도록 폰트 크기를 `28`에서 `32`로 다시 조정했다.
- 폰트 크기 증가에 맞춰 이름 텍스트 영역 높이도 `40`으로 함께 늘려, 줄림 없이 표시되도록 보정했다.
...
확인 내용:
- `dotnet build Assembly-CSharp.csproj` 빌드가 경고 0개, 오류 0개로 통과했다.
- 코드상으로 머리 위 이름 UI의 `fontSize`가 `32`, 텍스트 영역 높이가 `40`으로 반영된 것을 확인했다.

다음 작업:
- Unity Play 모드에서 실제 전투 중 플레이어끼리 가까워졌을 때 이름 겹침이 과하지 않은지 확인한다.
